...
class spPicture {
	 ...
	 * @param thumbnail		<b>psFormat</b> le format à rajouter
	 * @param commentaries	<b>array</b>
	 */
	public function addThumbnail($thumbnail,$commentaries)
	{
		if ($thumbnail->getFormat() === spFormatsTools::$original_pitures_format_id)
		{
			$this->thumbnails[spFormatsTools::$original_pitures_format_id]=$thumbnail;
		...
		else
		{
			...
			if( $this->label === null ) {
				$this->label = $this->id.".".spPicturesTools::getRealExtension($thumbnail->getFileName());
			}
		}
	}
	...
	{
		if ($format === spFormatsTools::$original_pitures_format_id)
		{
