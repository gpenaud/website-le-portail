...
class spPicturesDir {
	...
	private $pictures=array();	///< <b>array{psPicture}</b>
	...
	public function addThumbnail($picture_id,$picture_file_name,$format) {
		$thumbnail = new spThumbnail($picture_file_name,$format);
	...
	private function getPicture($picture_id) {
		...
		{
			$this->pictures[$picture_id] = new spPicture($picture_id);
		}
	...
	/**
	 * Méthode factory, permettant d'obtenir un objet {spPicturesDir} représentant les images trouvées
	...
	{
		$pictures_dir = new spPicturesDir($rep);

		$complet_dir_name = spMedia::getRootDevPath()."/".$rep;
		...
		if ($dir = @opendir($curent_dir)) {
			try {
				while (false !==  ($file = readdir($dir))) {
					...
					else if (is_file($curent_dir.'/'.$file) && spPicturesTools::isPictures($file))
					{
						$is_format = false;
						...
						{
							$pictures_dir->addThumbnail($m[1],$file,spFormatsTools::$original_pitures_format_id);
						}
					}
				}
				closedir($dir);
	...
	/**************************
	 * Méthodes pour obtenir le contenu de spPicturesDir
	 **************************/
	/**
	 * Retourne toutes les images trouvées ainsi que leur format.
	 * @return un tableau de {spPicture}
	 */
