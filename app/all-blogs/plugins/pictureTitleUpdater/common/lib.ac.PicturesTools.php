...
class ptuPicturesTools {
	...
	public static function getPicturesOfDirName($dir_name,$format_id,$all_formats)
	{
		...
		if ($format_id != ptuFormatsTools::$original_pitures_format_id
			&& !ptuFormatsTools::isFormat($format_id,$all_formats) )
		{
			return array();
		}

		$root_dir_name = ptuMedia::getRootDevPath()."/".$dir_name;
			...
			if (false !==  ($dir = @opendir($root_dir))) {
				try {
					while(false !==  ($file = readdir($dir))) {
						...
						if (is_file($root_dir.'/'.$file) && ptuPicturesTools::isPictures($file)) {
			...
			if ($dir = @opendir($root_dir.'/'.$under_dir)) {
				try {
					while($file = readdir($dir)) {
						...
						elseif (is_file($root_dir.'/'.$under_dir.'/'.$file) && ptuPicturesTools::isPictures($file))
						{
							if (preg_match('#^'.ptuFormatsTools::getOneFormat($format_id,$all_formats)->getUserExpr().'\.'.ptuPicturesTools::getRealExtension($file).'$#',$under_dir.$file,$m))
							{
								if ($if_option == true ) return true;
	...
	public static function isDirWithPictures($rep,$pictures_format_id,$all_formats)
	{
		...
		if  ($pictures_format_id != ptuFormatsTools::$original_pitures_format_id
			&& !ptuFormatsTools::isFormat($pictures_format_id,$all_formats))
		{
			return false;
		}

		$root_dir_name = ptuMedia::getRootDevPath()."/".$rep;
